...
| **Init-System** | systemd, OpenRC oder SysV init (automatisch erkannt) |
...
<Callout type="tip">
    Installation anpassen? Nutzen Sie Flags wie `--version 0.12.0`, `--install-dir /opt/bedrud` oder `--skip-shell`. Vollständige Referenz: [CLI-Installer](/de/docs/getting-started/cli-installer).
</Callout>
...
Bedrud in einem Container ausführen - kein Linux-Server-Setup, kein Init-System-Setup, kein Binär-Download.
...
Dies stoppt die Dienste, entfernt die Service-Dateien aller Init-Systeme, löscht die Binärdatei aus `/usr/local/bin` und bereinigt `/etc/bedrud`, `/var/lib/bedrud` und `/var/log/bedrud`.
