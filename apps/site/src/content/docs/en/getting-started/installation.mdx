...
| **Access** | Root or sudo |
| **Init system** | systemd, OpenRC, or SysV init (auto-detected) |
...
Run Bedrud in a container - no Linux server setup, no init system setup, no binary download.
...
This stops services, removes service files for all init systems, deletes the binary from `/usr/local/bin`, and cleans up `/etc/bedrud`, `/var/lib/bedrud`, and `/var/log/bedrud`.
