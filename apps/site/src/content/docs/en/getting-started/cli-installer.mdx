...
9. **Init system detection** — detects systemd, OpenRC, or SysV init (Linux only, for server setup)
10. **PATH check** — already in PATH? Skip. `--skip-shell`? Print manual command. Otherwise → modify RC file
11. **Completions** — run `bedrud completions`
12. **Done** — print success + reload instructions
