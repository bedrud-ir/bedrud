...
description: Instalar el servidor Bedrud en Linux
order: 2
---

Esta guía cubre la instalación del **servidor Bedrud** (API, interfaz web, servidor de medios WebRTC) en un servidor Linux. Para las aplicaciones cliente de escritorio y móvil, consulta [Instalación del Cliente](/es/docs/getting-started/clients).
...
| **Acceso** | Root o sudo |
| **Sistema init** | systemd, OpenRC o SysV init (detección automática) |
...
<Callout type="tip">
    ¿Personalizar la instalación? Usa flags como `--version 0.12.0`, `--install-dir /opt/bedrud` o `--skip-shell`. Referencia completa: [CLI Installer](/es/docs/getting-started/cli-installer).
</Callout>
...
Ejecuta Bedrud en un contenedor: sin configuración de servidor Linux, sin configuración de sistema de inicialización, sin descarga de binario.
...
**Requiere:** Python 3.12+, [uv](https://docs.astral.sh/uv/) (ejecutor de paquetes Python), acceso por clave SSH al servidor de destino, Go 1.24+ y Bun instalados localmente.
...
Esto detiene los servicios, elimina los archivos de servicio de todos los sistemas de inicialización, borra el binario de `/usr/local/bin` y limpia `/etc/bedrud`, `/var/lib/bedrud` y `/var/log/bedrud`.
...
| **Advertencias de certificado autofirmado** | Es lo esperado. Consulta la [Guía de TLS Interno](/es/docs/guides/internal-tls) para configuración de CA privada |
