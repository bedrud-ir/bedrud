...
| **权限** | Root 或 sudo |
| **初始化系统** | systemd、OpenRC 或 SysV init（自动检测） |
...
在容器中运行 Bedrud - 无需 Linux 服务器配置、无需 init 系统、无需下载二进制文件。
...
此操作会停止服务、删除所有 init 系统的服务文件、从 `/usr/local/bin` 中删除二进制文件，并清理 `/etc/bedrud`、`/var/lib/bedrud` 和 `/var/log/bedrud`。
