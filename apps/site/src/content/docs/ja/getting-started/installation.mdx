...
| **init システム** | systemd、OpenRC、SysV init（自動検出） |
...
コンテナで Bedrud を実行します。Linux サーバーのセットアップ、イニットシステムの設定不要、バイナリのダウンロードは不要です。
...
サービスの停止、すべてのイニットシステムのサービスファイルを削除、`/usr/local/bin` からのバイナリ削除、および `/etc/bedrud`、`/var/lib/bedrud`、`/var/log/bedrud` のクリーンアップを行います。
