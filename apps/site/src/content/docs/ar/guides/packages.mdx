...
تثبّت الحزمة الملف الثنائي في `/usr/local/bin/bedrud` وتُسجّل خدمة init (systemd/OpenRC/SysV كشف تلقائي). بعد التثبيت، شغّل المثبّت التفاعلي:

```bash
sudo bedrud install
```

راجع [دليل التثبيت](/ar/docs/getting-started/installation) لجميع خيارات الإعداد.
...
راجع [دليل Docker](/ar/docs/guides/docker) للتفاصيل الكاملة بما في ذلك تركيب الأقراص وإعداد الوكيل العكسي.
