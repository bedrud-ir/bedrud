...
| **الوصول** | Root أو sudo |
| **نظام init** | systemd أو OpenRC أو SysV init (كشف تلقائي) |
...
<Callout type="tip">
    تخصيص التثبيت؟ استخدم المعاملات مثل `--version 0.12.0` أو `--install-dir /opt/bedrud` أو `--skip-shell`. المرجع الكامل: [مُثبّت CLI](/ar/docs/getting-started/cli-installer).
</Callout>
...
شغّل بدرود في حاوية - بدون إعداد خادم لينكس، بدون إعداد نظام تهيئة، بدون تنزيل ثنائي.
...
يُوقف الخدمات، يزيل ملفات خدمة جميع أنظمة التهيئة، يحذف الملف الثنائي من `/usr/local/bin`، وينظّف `/etc/bedrud`، `/var/lib/bedrud`، و `/var/log/bedrud`.
