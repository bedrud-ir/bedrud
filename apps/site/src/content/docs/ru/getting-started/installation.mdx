...
| **Система инициализации** | systemd, OpenRC или SysV init (автоопределение) |
...
<Callout type="tip">
    Хотите настроить установку? Используйте флаги: `--version 0.12.0`, `--install-dir /opt/bedrud` или `--skip-shell`. Полная документация: [CLI Установщик](/ru/docs/getting-started/cli-installer).
</Callout>
...
Запустите Bedrud в контейнере - не требуется настройка Linux-сервера, система инициализации или загрузка бинарного файла.
...
Эта команда останавливает службы, удаляет сервис-файлы всех систем инициализации, удаляет бинарный файл из `/usr/local/bin` и очищает `/etc/bedrud`, `/var/lib/bedrud` и `/var/log/bedrud`.
