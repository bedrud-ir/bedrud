...
بسته باینری را در `/usr/local/bin/bedrud` نصب می‌کند و یک init سرویس (systemd/OpenRC/SysV تشخیص خودکار) ثبت می‌کند. پس از نصب، نصب‌کننده تعاملی را اجرا کنید:

```bash
sudo bedrud install
```

به [راهنمای نصب](/fa/docs/getting-started/installation) برای تمام گزینه‌های پیکربندی ببینید.
...
به [راهنمای Docker](/fa/docs/guides/docker) برای جزئیات کامل شامل سوار کردن حجم و تنظیم reverse proxy ببینید.
...
<Tabs>
  ...
  <TabsContent value="apt">
    ابتدا مخزن apt را اضافه کنید (به بالا [سرور - مخزن apt](#apt-repository-ubuntu-debian) ببینید)، سپس:
  ...
  <TabsContent value="dnf">
    ابتدا مخزن DNF را اضافه کنید (به بالا [سرور - مخزن DNF](#dnf-rpm-repository-fedora-rhel-opensuse) ببینید)، سپس:
...
<Callout type="info">
  انتشارهای macOS **امضاشده و تأییدشده با کد** هستند زمانی که `MACOS_CERTIFICATE` در CI پیکربندی شده است (به [مرجع اسرار](#secrets-reference) ببینید). بدون آن اسرار، باینری امضانشده نیست - از راه‌حل `xattr` بالا یا راست‌کلیک → باز در Finder استفاده کنید.
</Callout>
