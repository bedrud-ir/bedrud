...
| **دسترسی** | Root یا sudo |
| **سیستم init** | systemd، OpenRC یا SysV init (تشخیص خودکار) |
...
<Callout type="tip">
    نصب سفارشی؟ از پارامترهایی مثل `--version 0.12.0`، `--install-dir /opt/bedrud` یا `--skip-shell` استفاده کنید. مرجع کامل: [نصب‌کننده CLI](/fa/docs/getting-started/cli-installer).
</Callout>
...
بدرود را در یک container اجرا کنید — بدون نصب مستقیم روی سرور لینوکس، بدون تنظیم سیستم init، بدون دانلود باینری.
...
این سرویس‌ها را متوقف می‌کند، فایل‌های سرویس تمام سیستم‌های init را حذف می‌کند، باینری را از `/usr/local/bin` حذف می‌کند، و `/etc/bedrud`، `/var/lib/bedrud`، و `/var/log/bedrud` را پاک می‌کند.
