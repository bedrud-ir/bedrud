...
| **Accès** | Root ou sudo |
| **Système d'initialisation** | systemd, OpenRC ou SysV init (détection automatique) |
...
<Callout type="tip">
    Personnalisez l'installation ? Utilisez les flags comme `--version 0.12.0`, `--install-dir /opt/bedrud` ou `--skip-shell`. Référence complète : [CLI Installer](/fr/docs/getting-started/cli-installer).
</Callout>
...
Exécutez Bedrud dans un conteneur - pas de configuration de serveur Linux, pas de configuration de système d'initialisation, pas de téléchargement de binaire.
...
Cela arrête les services, supprime les fichiers de service de tous les systèmes d'initialisation, supprime le binaire de `/usr/local/bin` et nettoie `/etc/bedrud`, `/var/lib/bedrud` et `/var/log/bedrud`.
