...
2. [GitHub Sürümleri](https://github.com/bedrud-ir/bedrud/releases) adresinden doğru ikili dosyayı indirir (`--build` ile kaynak koddan derler)
...
| `--branch <name>` | `main` | Klonlanacak Git dalı (`--build` gerekir) |
...
Desteklenen: Çoğu Linux dağıtımı (systemd, OpenRC, SysV init), WSL1 ve Linux konteynerleri.
...
2. **Bağımlılık kontrolü** — `curl` + `tar` gerektirir (`--build` ile `git` + `go` + `bun` + `make` de)
3. **Argüman ayrıştırma** — `--build` ve `--branch` dahil bayraklar
4. **Platform algılama** — `uname` ile işletim sistemi + mimari
5. **Varyant algılama** — Rosetta 2
6. **İkili dosya kurulumu** — iki seçenek:
   - **İndirme modu** (varsayılan): `bedrud_{TARGET}.tar.xz` indir, çıkar, kur
   - **Derleme modu** (`--build`): `git clone --branch <branch> --depth 1`, `make build` çalıştır, ikili dosyayı kur
7. **Doğrulama** — `bedrud --version` komutunu çalıştır
8. **PATH kontrolü** — PATH'te zaten var mı? Atla. `--skip-shell`? Manuel komutu yazdır. Aksi halde → RC dosyasını düzenle
9. **Tamamlamalar** — `bedrud completions` komutunu çalıştır
10. **Tamamlandı** — başarı ve yeniden yükleme talimatlarını yazdır
