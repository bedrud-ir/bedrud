...
| **Başlatma sistemi** | systemd, OpenRC veya SysV init (otomatik algılanır) |
...
Bedrud'u bir konteynerde çalıştırın - Linux sunucu kurulumu, init sistemi kurulumu veya ikili dosya indirmesi gerekmez.
...
Bu, servisleri durdurur, tüm init sistemlerinin servis dosyalarını kaldırır, `/usr/local/bin` dizininden ikili dosyayı siler ve `/etc/bedrud`, `/var/lib/bedrud` ve `/var/log/bedrud` dizinlerini temizler.
