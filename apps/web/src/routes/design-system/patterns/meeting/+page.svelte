<script lang="ts">
  ...
  function setSpotlight(id: number | null) {
...
<div class="space-y-8">
  <div>
    ...
    <p class="text-muted-foreground mt-2">
      A comprehensive video conferencing interface with participant grid,
  ...
  <section class="space-y-6">
    ...
    <p class="text-muted-foreground mb-4">
      The meeting UI consists of three main sections: header with meeting info,
      video grid, and control bar. This pattern handles both RTL and LTR
      layouts, spotlight mode, and different participant states.
    </p>
    ...
    <Card.Root>
      ...
      <Card.Content>
        <div
          class={`border rounded-lg overflow-hidden ${direction === "rtl" ? "rtl" : ""}`}
        >
          <!-- Header Mock -->
          <div
            class="bg-background border-b p-3 flex justify-between items-center"
          >
            <div
              class={`flex items-center ${direction === "rtl" ? "space-x-reverse" : ""} space-x-4`}
            >
              <h3 class="font-medium">Meeting Title</h3>
              <div class="h-4 w-px bg-muted-foreground/30"></div>
              <div
                class={`flex items-center ${direction === "rtl" ? "space-x-reverse" : ""} space-x-1 text-muted-foreground text-xs`}
              >
            ...
            <div
              class={`flex items-center ${direction === "rtl" ? "space-x-reverse" : ""} space-x-2`}
            >
              <Button
                variant="ghost"
                size="sm"
              ...
              <Button variant="ghost" size="sm" class="h-8">
                <LogOut
                  class={`h-3.5 w-3.5 ${direction === "rtl" ? "ml-1" : "mr-1"}`}
                />
                Leave
              </Button>
            </div>
          ...
          {#if spotlightId !== null}
            {@const spotlighted = participants.find(
              (p) => p.id === spotlightId,
            )}
            <!-- Spotlight Mode -->
            <div class="bg-muted/20 p-3 h-[240px]">
              <div
                class="h-full rounded overflow-hidden bg-muted relative flex items-center justify-center"
              >
                {#if spotlighted?.isVideoOff}
                  <div class="flex flex-col items-center">
                    <div
                      class="w-16 h-16 rounded-full bg-muted-foreground/20 flex items-center justify-center mb-2"
                    >
                      <span class="text-lg">{spotlighted.name.charAt(0)}</span>
                    </div>
                    <span>{spotlighted.name}</span>
                  </div>
                {:else}
                  <div
                ...
                {#if spotlighted}
                  <div
                    class={`absolute bottom-2 ${direction === "rtl" ? "right-2" : "left-2"} bg-black/60 px-2 py-1 rounded text-white text-xs flex items-center`}
                  >
                    <span>{spotlighted.name}</span>
                    {#if spotlighted.isMuted}
                      <MicOff
                        class={`${direction === "rtl" ? "mr-1.5" : "ml-1.5"} h-3 w-3`}
                      />
                    {/if}
                  </div>
                {/if}
          ...
          {:else}
            ...
            <div class="bg-muted/20 p-3 grid grid-cols-2 gap-2 h-[240px]">
              {#each participants as participant}
                <div
                  class="relative rounded overflow-hidden bg-muted flex items-center justify-center"
                >
                  {#if participant.isVideoOff || (participant.id === 1 && videoOff)}
                    <div class="flex flex-col items-center">
                      <div
                        class="w-10 h-10 rounded-full bg-muted-foreground/20 flex items-center justify-center"
                      >
                  ...
                  <div
                    class={`absolute bottom-1 ${direction === "rtl" ? "right-1" : "left-1"} bg-black/50 px-1.5 py-0.5 rounded text-white text-xs flex items-center`}
                  >
                    <span
                      >{participant.id === 1 ? "You" : participant.name}</span
                    >
                    {#if participant.isMuted || (participant.id === 1 && muted)}
                      <MicOff
                        class={`${direction === "rtl" ? "mr-1" : "ml-1"} h-3 w-3`}
                      />
                    {/if}
                  </div>
          ...
          <!-- Controls Mock -->
          <div
            class="bg-background border-t p-3 flex justify-between items-center"
          >
            ...
            <div
              class={`flex-1 flex justify-center ${direction === "rtl" ? "space-x-reverse" : ""} space-x-2`}
            >
              <Button
              ...
              <Button variant="destructive" class="h-9 px-3">
                <LogOut
                  class={`h-4 w-4 ${direction === "rtl" ? "ml-1.5" : "mr-1.5"}`}
                />
                End
              </Button>
            </div>
            ...
            <div
              class={`flex-1 flex justify-end ${direction === "rtl" ? "space-x-reverse" : ""} space-x-2`}
            >
      ...
      <Tabs.Content value="header" class="pt-4">
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>
              Contains meeting title, participant count, and general meeting
              controls
            </Card.Description>
          </Card.Header>
          <Card.Content class="space-y-4">
            <div
              class="bg-background border rounded-lg p-3 flex justify-between items-center"
            >
              <div class="flex items-center space-x-4">
                <h3 class="font-medium">Meeting Title</h3>
                <div class="h-4 w-px bg-muted-foreground/30"></div>
                <div
                  class="flex items-center space-x-1 text-muted-foreground text-xs"
                >
                  <Users class="h-3.5 w-3.5" />
                  <span>4</span>
                </div>
          ...
          <Card.Footer class="flex justify-end border-t p-4">
            <Button
              ...
              onclick={() =>
                copyCode(`<header class="bg-background/95 backdrop-blur border-b">
    ...
    <Button variant="ghost" size="sm" onclick={leaveMeeting}>
      ...
      <Tabs.Content value="grid" class="pt-4">
        <Card.Root>
          ...
          <Card.Content class="space-y-4">
            <div
              class="bg-muted/20 p-3 grid grid-cols-2 gap-2 border rounded-lg h-[200px]"
            >
              {#each participants.slice(0, 2) as participant}
                ...
                >
                  ...
                  <div
                    class="absolute bottom-1 left-1 bg-black/50 px-1.5 py-0.5 rounded text-white text-xs flex items-center"
                  >
          ...
          <Card.Footer class="flex justify-end border-t p-4">
            <Button
              ...
              class="flex items-center gap-1"
              onclick={() =>
                copyCode(`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 h-full auto-rows-fr">
        ...
        <Card.Root>
          ...
          <Card.Content class="space-y-4">
            ...
            <div class="text-sm text-muted-foreground space-y-2">
              ...
              <ul class="list-disc pl-5 space-y-1">
                ...
                <li>
                  Show the most important controls prominently in the center
                </li>
          ...
          <Card.Footer class="flex justify-end border-t p-4">
            <Button
              ...
              class="flex items-center gap-1"
              onclick={() =>
                copyCode(`<div class="border-t bg-background/95 backdrop-blur">
      ...
      <Card.Root>
        ...
        <Card.Content>
          <ul class="list-disc pl-5 space-y-2 text-muted-foreground">
            ...
            <li>
              Maintain 16:9 aspect ratio for video containers when possible
            </li>
      ...
      <Card.Root class="md:col-span-2">
        ...
        <Card.Content>
          <ul class="list-disc pl-5 space-y-2 text-muted-foreground">
            ...
            <li>
              Provide keyboard shortcuts for common actions (mute, video toggle)
            </li>
            ...
            <li>
              Support screen readers with appropriate notifications for meeting
              events
            </li>
          </ul>
        </Card.Content>
  ...
  <section class="space-y-6">
    ...
    <p class="text-muted-foreground">
      For a complete implementation, refer to the meeting page component
      available at <code>src/routes/m/[meetId]/+page.svelte</code>. This
      component demonstrates handling video streams, participant management, and
      meeting controls.
    </p>
      ...
      <Card.Content>
        <ul class="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <li class="p-2 border rounded-md">
            ...
            <div class="text-sm text-muted-foreground">
              For organizing participant tiles
            </div>
          </li>
          <li class="p-2 border rounded-md">
            <div class="font-medium">State Indicators</div>
            <div class="text-sm text-muted-foreground">
              For mute/video status
            </div>
          </li>
        </ul>
      </Card.Content>
