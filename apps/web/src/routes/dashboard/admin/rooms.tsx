...
import { Search, Globe, Lock, Activity, ChevronDown, ChevronUp, Power } from 'lucide-react'
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[560px]">
            <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-3 border-b bg-muted/30 px-4 py-2.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
              ...
              <button className="text-left hover:text-foreground transition-colors hidden sm:block" onClick={() => toggleSort('createdAt')}>
              ...
              {isLoading ? (
                [...Array(4)].map((_, i) => (
                  <div key={i} className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-3 px-4 py-3 animate-pulse">
                    {[...Array(6)].map((__, j) => <div key={j} className="h-3.5 rounded-full bg-muted" />)}
                  </div>
                ))
              ) : rooms.length === 0 ? (
                ...
                >
                  ...
                  {editingLimit === room.id ? (
                    <input
                      className="w-14 rounded-md border border-primary/40 bg-primary/5 px-1.5 py-0.5 text-xs text-center outline-none"
                      value={limitValue}
                      type="number"
                      min={1}
                  ...
                  {confirmClose === room.id ? (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="rounded-md bg-destructive px-2 py-1 text-[10px] font-semibold text-destructive-foreground transition-opacity hover:opacity-80 disabled:opacity-50"
                      >
                        Confirm
                      </button>
                      <button
                    ...
                    <button
                      ...
                      className="rounded-md p-1.5 transition-colors hover:bg-destructive/10 hover:text-destructive disabled:opacity-30 disabled:cursor-not-allowed text-muted-foreground"
