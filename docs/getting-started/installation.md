...
This guide covers installing the **Bedrud server** (API, web UI, WebRTC media server) on a Linux server. For desktop and mobile client apps, see [Client Installation](clients.md).
...
> **No domain?** Bedrud works with an IP address using self-signed TLS. Browsers will show a certificate warning but video and audio work. For trusted certificates on internal networks, see the [Internal TLS Guide](../guides/internal-tls.md).
...
!!! tip
    For desktop client installation (Linux, macOS, Windows) see [Client Installation](clients.md).
...
Full Docker documentation including volumes, reverse proxy, and TLS setup: [Docker Guide](../guides/docker.md).
...
| 5 | Creates two systemd services: `bedrud.service` and `livekit.service` |
...
> **Why promote?** The first registered user is a regular user by default — not an admin. This is a security measure: if your server is publicly accessible, anyone could register before you. Promoting explicitly ensures only you get admin access.
...
| **Self-signed cert warnings** | Expected. See [Internal TLS Guide](../guides/internal-tls.md) for private CA setup |
